...
def generate_suggestions(model_path, template_dir, input_series, output_dir, timeout_seconds):
    chat_handler = hf_autotokenizer_to_chat_completion_handler("mistralai/Mistral-Nemo-Instruct-2407")
    model = Llama(model_path=model_path, n_gpu_layers=-1, n_ctx=2048, chat_handler=chat_handler)

    output = {}
    try:
        logger.info(f'Generating suggestions')
        for palga_input in input_series:
            output[palga_input] = _suggestion_for_input(model, palga_input)
...
def _suggestion_for_input(model, palga_input):
