...
class DatabaseWriterTest {
    ...
    private val exhaustiveDiagnosisYear = TestReferencePatientFactory.exhaustiveReferencePatient().tumors[0].diagnosisYear
    ...
    private fun compare(expectedPatients: List<ReferencePatient>) {
        ...
        expectedPatients.mapIndexed { index, patient ->
            ...
            patient.tumors.firstOrNull { it.diagnosisYear == exhaustiveDiagnosisYear }?.let { expectedTumor ->
                val expectedTumorRecord = dslContext.selectFrom(Tables.TUMOR)
                    .where(Tables.TUMOR.PATIENTID.eq(patientId).and(Tables.TUMOR.DIAGNOSISYEAR.eq(exhaustiveDiagnosisYear))).fetchOne()
    ...
    private fun compareTumor(record: TumorRecord, expectedTumor: Tumor) {
        ...
        expectedTumor.priorTumors.mapIndexed { index, priorTumor ->
            ...
            comparePriorTumor(priorTumorRecord!!, priorTumor)
        }
